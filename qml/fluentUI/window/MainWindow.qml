...
FluWindow {
    ...
    FluContentDialog {
        ...
        message: qsTr("FluentUI is currently up to date ") + newVerson + qsTr(
                     " -- The current app version") + AppInfo.version + qsTr(
        ...
        onPositiveClicked: {
            Qt.openUrlExternally(
                        "https://github.com/zhuzichu520/FluentUI/releases/latest")
        }
    }
    ...
    FluNetworkCallable {
        ...
        onSuccess: result => {
                       ...
                       if (data.tag_name !== AppInfo.version) {
                           ...
                           if (!silent) {
                               showInfo(
                                   qsTr("The current version is already the latest"))
                           }
                       }
                   }
    ...
    function checkUpdate(silent) {
        ...
        FluNetwork.get(
                    "https://api.github.com/repos/zhuzichu520/FluentUI/releases/latest").go(
                    callable)
    }
}
