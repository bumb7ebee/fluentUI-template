...
std::map<QtMsgType, int> logLevelMap = {{QtFatalMsg, 0},
                                        {QtCriticalMsg, 1},
                                        {QtWarningMsg, 2},
                                        {QtInfoMsg, 3},
                                        {QtDebugMsg, 4}};
...
QString Log::prettyProductInfoWrapper() {
  ...
  auto macosVersionFile = QString::fromUtf8(
...
#if defined(Q_OS_WIN)
  QSettings regKey{
      QString::fromUtf8("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows "
                        "NT\\CurrentVersion"),
      QSettings::NativeFormat};
  if (regKey.contains(QString::fromUtf8("CurrentBuildNumber"))) {
    auto buildNumber =
        regKey.value(QString::fromUtf8("CurrentBuildNumber")).toInt();
    if (buildNumber > 0) {
      if (buildNumber < 9200) {
...
static inline void messageHandler(const QtMsgType type,
    ...
    if (!g_logFile) {
      ...
      if (!g_logFile->open(QFile::WriteOnly | QFile::Text | QFile::Append)) {
        std::cerr << "Can't open file to write: "
                  << qPrintable(g_logFile->errorString()) << std::endl;
        g_logFile.reset();
        g_logError = true;
        return;
      }
...
void Log::setup(char *argv[], const QString &app, int level) {
  ...
  const QString logFileName =
      QString("%1_%2.log")
          .arg(g_app, QDateTime::currentDateTime().toString("yyyyMMdd"));
